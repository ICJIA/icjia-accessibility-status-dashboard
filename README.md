...
- **📖 [Full Documentation Site](http://localhost:3002)** - Comprehensive guides, API reference, and deployment instructions
- **🚀 [Getting Started Guide](http://localhost:3002/docs/intro)** - Step-by-step setup instructions
- **🔌 [API Reference](http://localhost:3002/docs/api/overview)** - Complete API documentation with examples
- **🚢 [Deployment Guides](http://localhost:3002/docs/deployment/overview)** - Production deployment options
- **🔧 [Troubleshooting](http://localhost:3002/docs/troubleshooting/common-issues)** - Solutions to common problems
- **🔒 [Security & Audits](http://localhost:3002/docs/security/audit-overview)** - Security findings and audit reports

### Documentation Site

Complete documentation is available at **http://localhost:3002** (development) or **https://accessibility.icjia.app/docs** (production).

The documentation site is built with **Docusaurus 3** and includes:

| Section                  | Contents                                                   |
| ------------------------ | ---------------------------------------------------------- |
| **🚀 Getting Started**   | Quick start, setup guide, and introduction                 |
| **🏗️ Core Concepts**     | Authentication, database schema, and architecture          |
| **💻 Development**       | Development setup, reset scripts, and testing              |
| **⚙️ Configuration**     | Environment variables and configuration guide              |
| **🔌 API Reference**     | Complete API documentation with examples                   |
| **🚢 Deployment**        | Production deployment and infrastructure setup             |
| **🔒 Security & Audits** | Security findings, audit reports, and Docker configuration |
| **🔧 Troubleshooting**   | Solutions to common problems                               |

### Accessing the Documentation

**When running `yarn dev`, all three services start automatically:**

```
Frontend:       http://localhost:5173 (opens in browser)
Backend API:    http://localhost:3001/api
Documentation:  http://localhost:3002 (Docusaurus portal)
```

**To start only the documentation site:**
...
**Access docs from the main app:**

Click the **"Docs"** link in the navigation bar (only visible when logged in) to open the documentation site in a new tab.

**Documentation Portal Features:**

- ✅ **Official ICJIA Logo** in navbar (same as frontend)
- ✅ **Emoji-enhanced sidebar** with category icons for easy navigation
- ✅ **Dark theme** with green accents (Nuxt-inspired design)
- ✅ **Responsive design** for mobile, tablet, and desktop
- ✅ **Health check indicator** in navbar showing system status
- ✅ **Search functionality** for finding documentation quickly
- ✅ **Monorepo integration** - Single `yarn install` installs all dependencies
...
All domain names and ports are configured via the `.env` file. Changes to `.env` automatically ripple through the entire application:
...
The `.env` file contains **17 environment variables** organized into 8 sections:

**Required (4):**

- `VITE_SUPABASE_URL` - Supabase project URL
- `VITE_SUPABASE_ANON_KEY` - Supabase anon key
- `FRONTEND_URL` - Frontend domain (used for CORS)
...
- Rate limiting variables (8)
- API key rotation variables (2)
...
### .env and .env.sample Synchronization

- **`.env`** - Contains actual sensitive values (API keys, real credentials)
- **`.env.sample`** - Contains placeholders for all variables (safe to commit)

Both files have **identical structure** with only values differing. When adding new environment variables:

1. Add to both `.env` and `.env.sample`
2. Use actual values in `.env`
3. Use placeholders in `.env.sample`
4. Keep comments and documentation identical

**⚠️ Database Reset Scripts** (Development Only):

Two powerful reset scripts are available for development:

1. **`yarn reset:users`** - Reset admin access only

   - Deletes all admin users, API keys, and sessions
   - **Preserves** all sites, score history, API payloads, and activity logs
   - Use when: You need to reset admin credentials or clear API keys
   - Requires two confirmation prompts to prevent accidental execution

2. **`yarn reset:app`** - Complete database wipe
   - Deletes **everything**: users, API keys, sessions, sites, history, logs, documentation
   - Returns database to completely empty state
   - Use when: Starting completely from scratch
   - Requires two confirmation prompts to prevent accidental execution

**See `/docs/RESET_SCRIPTS.md` for detailed documentation on these scripts.**

**Database Migrations** (see `/supabase/migrations/README.md` for details):

- **STEP 1**: `step_1_create_initial_schema.sql` - Initial schema with sample data
- **STEP 2**: `step_2_api_keys_and_rls_fixes.sql` - API keys and RLS policies

---

## 🐳 Docker Setup

The application can be run entirely in Docker with all three services (frontend, backend, documentation) orchestrated via Docker Compose.

### Quick Start with Docker

```bash
# 1. Copy environment file and add Supabase credentials
cp .env.sample .env
...
| ----------------- | ------ | -------------------- | ---------------- |
| **Frontend**      | 5173   | React 18 + Vite      | ✅ Containerized |
| **Backend**       | 3001   | Express.js + Node.js | ✅ Containerized |
| **Documentation** | 3002   | Docusaurus 3         | ✅ Containerized |
| **Nginx Proxy**   | 80/443 | Nginx Alpine         | ✅ Configured    |

### Docker Files

- **`Dockerfile.frontend`** - React/Vite frontend with Nginx
- **`Dockerfile.backend`** - Express.js backend with Node.js
- **`Dockerfile.docs`** - Docusaurus documentation with Nginx
...
For detailed Docker setup, troubleshooting, and production deployment:

📖 **[Docker Audit Report](http://localhost:3002/docs/deployment/docker-audit-report)** - Complete Docker configuration review and validation
...
Complete deployment guides are available in the documentation:

### Laravel Forge Deployment

**File:** `docs/docs/deployment/laravel-forge.md`

Complete guide for deploying on Laravel Forge with:
...
**File:** `docs/docs/deployment/coolify-deployment.md`

Complete guide for deploying on Coolify (self-hosted or managed) with:

- ✅ Coolify installation options
...
- [ ] Set `VITE_API_URL` to your production domain with `/api`
- [ ] Set `VITE_DOCS_URL` to `/docs/` (served from same domain)
...
**First time setting up this project?** This comprehensive guide will walk you through every step from creating a Supabase project to deploying on a production server.

> **💡 Quick Setup:** This project uses **three migration files** that set up everything - tables, security, API keys, admin user, and sample data. No complex migration sequences!
...
3. Fill in the project details:
   - **Name**: `icjia-accessibility-portal` (or any name you prefer)
   - **Database Password**: Create a strong password and **save it securely**
     - Example: `MySecurePass123!@#` (use your own!)
     - You'll need this if you ever need direct database access via psql or pgAdmin
     - This is NOT the admin user password for the application
   - **Region**: Choose the region closest to your users
     - For ICJIA: `East US (North Virginia)` is recommended
     - For development: Choose closest to your location
   - **Pricing Plan**: Select "Free" for development (includes 500MB database, 2GB bandwidth)
4. Click **"Create new project"**
5. Wait 1-2 minutes for Supabase to provision your database
6. You'll see a success message when your project is ready
...
Open `.env` in your text editor and update with your Supabase credentials:

```bash
# Supabase Configuration (UPDATE THESE!)
VITE_SUPABASE_URL=https://your-project-id.supabase.co
VITE_SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlvdXItcHJvamVjdC1pZCIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNjM4MzE2ODAwLCJleHAiOjE5NTM4OTI4MDB9.YOUR_ACTUAL_ANON_KEY_HERE
...
- Replace `VITE_SUPABASE_URL` with your actual Project URL from step 3
- Replace `VITE_SUPABASE_ANON_KEY` with your actual Anon key from step 3
- Keep the other values as-is for local development
- The `.env` file is git-ignored and will never be committed

#### 5. Set Up the Database - THREE MIGRATION FILES

You need to run **THREE SQL files** in order. Each migration builds on the previous one.

##### STEP 1: Create Initial Schema (Required)

1. In your Supabase dashboard, click **SQL Editor** in the left sidebar
2. Click **"New query"** button
3. Open the file `supabase/migrations/step_1_create_initial_schema.sql` from this project
4. Copy **ALL** the contents (all 646 lines - use Cmd+A / Ctrl+A, then Cmd+C / Ctrl+C)
...
NOTICE: Created sample uploaded files for audit trail
NOTICE: Created documentation sections
NOTICE: Database setup complete!
NOTICE: Admin user created with BLANK password - you must set it on first login
```

**What this migration does:**

- ✅ Creates all 6 database tables (`admin_users`, `sessions`, `sites`, `score_history`, `app_documentation`, `uploaded_files` - later renamed to `api_payloads`)
- ✅ Sets up Row Level Security (RLS) policies configured for custom cookie-based authentication
- ✅ Creates the admin user with blank password (you'll set it on first login)
- ✅ Adds 6 sample websites with realistic data
- ✅ Generates 36 score history records (6 months per site)
- ✅ Creates 18 sample uploaded files (audit trail demonstration)
- ✅ Adds 3 documentation sections
- ✅ Sets up triggers for automatic timestamps

##### STEP 2: Add API Keys and Fix RLS Policies (Required)

**⚠️ Important**: Run this migration **AFTER** STEP 1.

1. In SQL Editor, click **"New query"** button
2. Open the file `supabase/migrations/step_2_api_keys_and_rls_fixes.sql` from this project
...
NOTICE: ✅ Created 4 RLS policies for api_keys table
NOTICE: ✅ admin_users UPDATE policy fixed
NOTICE: ✅ sites RLS policies fixed (3 policies)
...
- ✅ Fixes `uploaded_files` RLS policy to allow API audit trail (table later renamed to `api_payloads`)
- ✅ Adds indexes for performance
- ✅ Sets up RLS policies for API key management

##### STEP 3: Refactor to API-Only Data Ingestion (Optional)

**⚠️ Important**: Run this migration **AFTER** STEP 1 and STEP 2 (only if upgrading).

1. In SQL Editor, click **"New query"** button
2. Open the file `supabase/migrations/step_3_refactor_to_api_payloads.sql` from this project
3. Copy **ALL** the contents and paste into the Supabase SQL Editor
4. Click **"Run"** (or press Cmd+Enter / Ctrl+Enter)
5. You should see output like this:
...
NOTICE:   - Renamed uploaded_files → api_payloads
NOTICE:   - Added description field for commit-style messages
NOTICE:   - Added api_key_id to track which API key was used
NOTICE:   - Added ip_address and user_agent for audit trail
NOTICE:   - Updated RLS policies for admin-only viewing
NOTICE:   - Created activity_log table for event tracking
```

**What this migration does:**

- ✅ Renames `uploaded_files` table to `api_payloads` (reflects API-only approach)
- ✅ Adds `description` field for Git commit-style messages on API uploads
- ✅ Adds `api_key_id`, `ip_address`, `user_agent` for comprehensive audit trail
- ✅ Creates `activity_log` table for tracking all significant events
- ✅ Updates RLS policies for admin-only payload viewing
- ✅ Migrates existing data to new schema

**Why three migrations?**

1. **Migration 1**: Creates the core database structure with sample data
2. **Migration 2**: Adds API key functionality and fixes RLS policies
3. **Migration 3**: Refactors to API-only approach with enhanced audit trail
...
2. You should see **8 tables**:
   ...
   - `admin_users` (1 row)
   - `sessions` (0 rows - will populate when you login)
   - `sites` (6 rows - sample sites)
   - `score_history` (36 rows - historical data)
   - `app_documentation` (3 rows - documentation sections)
   - `api_payloads` (18 rows - sample API uploads - renamed from `uploaded_files`)
   - `api_keys` (0 rows - you'll create keys in the admin panel)
   - `activity_log` (0 rows - will populate with events)

3. Click on the **`admin_users`** table and verify you see:

   - **username**: `admin`
   - **email**: `admin@icjia.illinois.gov`
   - **password_hash**: Should be blank/empty (you'll set it on first login)
   - **must_change_password**: Should be `true`

4. Click on the **`api_payloads`** table and verify you see 18 rows (sample data migrated from `uploaded_files`)

5. Click on the **`sites`** table and verify you see 6 sample sites

6. Click on the **`api_keys`** table and verify it exists (should be empty initially)

If you see all this, your database is set up correctly! ✅
...
You should see the system status indicators in the top-right corner showing:

- 🟢 Backend: Running
- 🟢 Database: Connected

#### 8. Set Up Your Admin Password

1. Your browser should automatically open to `http://localhost:5173`
2. **The app will automatically detect that initial setup is needed** and redirect you to the setup page
3. You'll see the **"Initial Setup"** screen with a password form
4. Enter a secure password that meets all requirements:
...
7. You'll be redirected to the login page with a success message
8. Log in with:
   - **Username**: `admin`
   - **Your new password**
...
The app automatically checks if initial setup is needed when it loads. If the admin password is blank (initial state), you'll be redirected to the setup page **before** you can access any other part of the application. This ensures:

- ✅ **No manual navigation needed** - The app takes you directly to setup
- ✅ **Can't accidentally skip setup** - All routes redirect to setup until password is set
- ✅ **Clear user experience** - You know exactly what to do on first run

**🔒 Blank Password Security Approach:**

For security, the initial admin account is created with a **blank/empty password**. This approach has several important benefits:

- ✅ **No hardcoded passwords in the repository** - The migration file doesn't contain any actual passwords
- ✅ **No default credentials to remember** - Developers don't need to look up a default password
- ✅ **Forced password setup** - You cannot access any admin features until you set a real password
- ✅ **One-time blank password** - Once you set a password, you can never change it back to blank
- ✅ **GitHub security** - No risk of exposing default passwords in version control

The system **requires** you to set a password on first login. You cannot access any admin features until you set a secure password.

### Troubleshooting

#### "Admin user NOT found" error when logging in
...
If it shows "Admin user NOT found", you need to re-run the migration file `supabase/migrations/step_1_create_initial_schema.sql` in the Supabase SQL Editor.

#### "Failed to create session" error when logging in

This means the database wasn't set up correctly. Re-run the migration file `supabase/migrations/step_1_create_initial_schema.sql` in the Supabase SQL Editor. The file is idempotent (safe to run multiple times).

#### Backend won't start

Check that:

- Your `.env` file exists and has the correct Supabase credentials
- Port 3001 is not already in use by another application
- You have Node.js 20+ installed (run `node --version` to check)

#### Database connection errors

Check that:

- Your `VITE_SUPABASE_URL` in `.env` is correct (no trailing slash)
- Your `VITE_SUPABASE_ANON_KEY` in `.env` is the full key (very long string)
- Your Supabase project is active (check the dashboard)

#### System status shows "Database: Disconnected"

This usually means:

- Your Supabase credentials in `.env` are incorrect
- Your Supabase project is paused (free tier projects pause after inactivity)
- You need to restart the dev server after changing `.env`

### Initial Admin Account

After running the migration, the default admin account is created with:

- **Username**: `admin`
- **Password**: _(blank/empty)_

**🔒 BLANK PASSWORD SECURITY APPROACH:**

The system uses a **blank password** for the initial admin account. This is a security best practice that:

- ✅ **Prevents hardcoded passwords in the repository** - No actual passwords stored in the migration file
- ✅ **Eliminates default credential risks** - No "admin/admin123" or similar common defaults
- ✅ **Forces immediate password setup** - You cannot use the system without setting a real password
- ✅ **Protects against GitHub exposure** - No risk of exposing default passwords in version control
- ✅ **One-time use only** - Once you set a password, you can never change it back to blank

**How It Works:**

1. **Automatic Detection**: When you first open the app, it automatically detects that the admin password is blank
2. **Auto-Redirect to Setup**: You are immediately redirected to the "Initial Setup" page (you don't need to go to login first)
3. **Set Your Password**: Enter a secure password that meets all requirements
4. **Confirm Password**: Re-enter your password to confirm
5. **Complete Setup**: Click "Set Password" to save your new password
6. **Login**: You'll be redirected to the login page - log in with username `admin` and your new password
7. **Subsequent Changes**: After initial setup, password changes require entering your current password
...
- Create additional admin users with unique credentials
- Never commit the `.env` file to version control
- Use strong, unique passwords for all admin accounts
- The blank password approach is only for initial setup - all subsequent passwords must be secure

### Next Steps

Now that you're set up:

1. **Explore the dashboard** - View the sample sites at `http://localhost:5173`
2. **Log in to admin panel** - Go to `/login` and set your password (first-time setup)
3. **Create an API key** - Navigate to `/admin/api-keys` and create your first API key
4. **Import data via API** - Use the API import endpoint to add sites (see [API Import Endpoint](#api-import-endpoint))
5. **Test the API** - Run `node verify-api.js YOUR_API_KEY` to test the import endpoint
6. **Customize** - Modify the code to fit your needs
7. **Deploy** - See [Deployment Options](#deployment-options) for production deployment instructions

### Additional Resources

- **[API Documentation](#api-import-endpoint)** - Complete API reference for programmatic data import
- **[Deployment Options](#deployment-options)** - Multiple deployment methods (Ubuntu, Coolify, Vercel, etc.)
- **[Test Script](./verify-api.js)** - Example script for testing the API import endpoint
- **Diagnostic Tools**:
  - `yarn tsx diagnose-auth.ts` - Test authentication setup
  - `yarn seed` - Alternative way to seed the database from Node.js

---

## Authentication System

This application uses a **custom cookie-based authentication system** (not Supabase Auth). Understanding how it works is important for development and troubleshooting.
...
- ❌ Supabase Auth (`auth.signIn()`, `auth.signUp()`, etc.)
- ❌ Bearer tokens
- ❌ OAuth providers
...
```typescript
// 1. Query admin_users table for username
...
// 3. Generate cryptographically random session token
const sessionToken = crypto.randomBytes(32).toString("hex");

// 4. Store session in database (expires in 15 days)
...
// 6. Return user data (including must_change_password flag)
return res.json({
  user: {
    id,
    username,
...
**Result**: Browser automatically stores the cookie and sends it with every subsequent request.

**Forced Password Setup**: If `must_change_password` is `true`, the user is immediately redirected to `/change-password` and cannot access any other pages until they set a password.

#### 2. Initial Setup Flow (Automatic Detection)

When the app first loads, it automatically checks if initial setup is needed:

**Setup Check Component** (`src/components/SetupCheck.tsx`):

```typescript
// On app load, check if admin password is blank
const response = await api.auth.needsSetup();

if (response.needsSetup) {
  // Redirect to setup page before showing any other content
  navigate("/setup", { replace: true });
}
```

**Backend Check** (`server/routes/auth.ts`):
...
router.get("/needs-setup", async (req, res) => {
  ...
  // Setup is needed if admin doesn't exist or password is blank
  const needsSetup =
...
**Initial Setup Page** (`src/pages/InitialSetup.tsx`):

- Dedicated page for first-time password setup
- Does NOT require authentication (accessible before login)
- Validates password requirements in real-time
- Shows visual feedback for each requirement
- Prevents using blank/empty passwords
- Enforces strong password policy

**Backend Initial Setup** (`server/routes/auth.ts`):

```typescript
// POST /api/auth/initial-setup
router.post("/initial-setup", async (req, res) => {
  // 1. Validate new password meets requirements
  // 2. Get admin user and verify password is currently blank
  ...
  // 3. Hash new password with bcrypt (10 salt rounds)
  const newPasswordHash = await bcrypt.hash(newPassword, 10);

  // 4. Update password and clear must_change_password flag
  await supabase
    .from("admin_users")
    .update({
...
2. App detects blank admin password
3. User is automatically redirected to `/setup`
4. User sets a secure password
5. User is redirected to `/login` with success message
6. User logs in with new credentials

#### 3. Password Change Flow (After Initial Setup)

When a user needs to change their password after initial setup:
...
if (response?.user?.must_change_password) {
  ...
  navigate("/admin"); // Normal dashboard access
}
```

**Password Change Page** (`src/pages/ChangePassword.tsx`):
...
router.post("/change-password", requireAuth, async (req, res) => {
  ...
  // 5. Clear all sessions (force re-login)
  await supabase.from("sessions").delete().eq("user_id", userId);

  // 6. Clear current session cookie
...
```typescript
// Prevent access to any page if password change is required
...
fetch("/api/sites", {
  credentials: "include", // Automatically sends session_token cookie
});
```
...
**Protected Routes** (`server/routes/*.ts`):
...
// Uses requireAuth middleware to validate session cookie
...
**Important**: The application uses Supabase's **Anon Key** for ALL database operations, not user-specific JWT tokens.
...
export const supabase = createClient(
  ...
  process.env.VITE_SUPABASE_ANON_KEY! // Same key for all operations
);
```

**How this works securely**:

1. **Row Level Security (RLS)** policies control data access, not JWT tokens
2. RLS policies are configured to allow `anon` role to:
   - Read `admin_users` (for authentication)
   - Create/read/delete `sessions` (for session management)
   - Read `sites` (public data)
3. Write operations on `sites` are protected by the `requireAuth` middleware on the backend
4. The backend validates the session cookie before allowing any write operations
...
- ✅ **HttpOnly cookies** - JavaScript cannot access (prevents XSS attacks)
- ✅ **Secure flag** in production - HTTPS only (prevents MITM attacks)
- ✅ **SameSite: lax** - CSRF protection
...
- ✅ **Anon key** has limited permissions (can't access Supabase admin functions)

### Why Custom Auth Instead of Supabase Auth?

**Reasons for custom authentication**:

1. **Full control** over user management and session handling
2. **Custom user table** with application-specific fields
3. **Cookie-based sessions** instead of JWT in localStorage (more secure for web apps)
4. **Simpler integration** with existing admin workflows
5. **No dependency** on Supabase Auth pricing/limits

**Trade-offs**:

- ❌ No built-in OAuth providers (Google, GitHub, etc.)
- ❌ No magic link authentication
- ❌ No built-in email verification
- ✅ But: Full control and simpler codebase for admin-only application

### Row Level Security (RLS) Configuration

The RLS policies are specifically designed for custom authentication:

```sql
-- Allow anonymous users to read admin_users (for authentication)
CREATE POLICY "Allow anonymous read for authentication"
  ON admin_users FOR SELECT
  TO anon, authenticated
  USING (true);

-- Allow anonymous users to manage sessions (for login/logout)
...
- The app uses the Supabase Anon Key for all operations
- Authentication happens via session cookies, not Supabase JWT tokens
- RLS policies allow the necessary operations for custom auth to work
- Security is enforced by the backend middleware, not RLS alone
...
- Check that the migration file was run (creates admin user)
- The initial admin account has a BLANK password - you must set it on first login via the Initial Setup page
- Run `yarn tsx diagnose-auth.ts` to check database
- If needed, re-run `supabase/migrations/step_1_create_initial_schema.sql`

#### "Failed to create session" error

- Database migration may not have completed successfully
- Re-run `supabase/migrations/step_1_create_initial_schema.sql` (safe to run multiple times)
...
## Supabase Setup (Alternative Methods)

The quick start above covers the recommended manual setup. If you're setting up your own instance or need to create a separate development database, you can also use these alternative methods:
...
2. Click "New Project" in your organization
3. Fill in the project details:
   - **Project Name**: Choose any name (e.g., "icjia-accessibility-portal")
...
1. Go to your project's **Settings** (gear icon in the sidebar)
2. Navigate to **API** section
...
The `.env.sample` file contains all required environment variable keys with placeholder values. Update the following values with your actual Supabase credentials:
...
**Note**: The `.env` file is git-ignored and should never be committed. Always use `.env.sample` as a template.

### 4. Run Database Migrations - TWO REQUIRED FILES

The project includes **two required migration files** that set up **EVERYTHING** you need:

**Files:**

1. `supabase/migrations/step_1_create_initial_schema.sql` (REQUIRED - Run First)
2. `supabase/migrations/step_2_api_keys_and_rls_fixes.sql` (REQUIRED - Run Second)

These files create:

- ✅ **All database tables** (admin_users, sessions, sites, score_history, app_documentation, api_payloads)
- ✅ **Row Level Security (RLS)** policies for all tables
- ✅ **Admin user** with blank password (username: `admin`, must set password on first login)
- ✅ **Sample data** for testing (6 sites, 36 history records, 18 API payload records)
- ✅ **API keys table** for API authentication
- ✅ **Triggers and functions** for automatic timestamps
...
**Option B: Manual Migration via SQL Editor (Recommended for First-Time Setup)**
...
7. Repeat steps 3-6 for `supabase/migrations/step_2_api_keys_and_rls_fixes.sql`

**That's it!** Two files, two clicks, complete setup.
...
2. You should see these tables:
   ...
   - `sites` - Website tracking data (6 sample sites)
   - `score_history` - Historical accessibility scores (36 records)
   - `app_documentation` - Documentation sections (3 sections)
   - `api_payloads` - Audit trail of API uploads (18 sample records)
   - `api_keys` - API key management (initially empty)
   - `activity_log` - Event tracking (initially empty)
...
-- Check API payloads
SELECT COUNT(*) FROM api_payloads;
-- Expected: 18

-- Check API keys table exists
SELECT COUNT(*) FROM api_keys;
-- Expected: 0 (you'll create keys in the admin panel)
```
...
- **Username**: `admin`
- **Password**: Blank (empty) - **you must set a password on first login**

**Security Note**: The blank password approach ensures no default credentials are exposed in the codebase. You will be automatically redirected to the Initial Setup page to set a secure password when you first access the application.

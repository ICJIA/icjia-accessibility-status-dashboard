 ...
 * Run with: npx ts-node scripts/test-axe.ts <url>
 * Example: npx ts-node scripts/test-axe.ts https://dvfr.illinois.gov/
 */
...
async function testAxeScan() {
  ...
  try {
    ...
    // Try using addScriptTag instead of addInitScript
    ...
    if (!axeAvailable) {
      console.log("\n[DEBUG] Checking window object...");
      const windowKeys = await page.evaluate(() => {
        return Object.keys(window as any).filter(
          (k) => k.includes("axe") || k.includes("Axe")
        );
      });
      console.log("Window keys containing 'axe':", windowKeys);
